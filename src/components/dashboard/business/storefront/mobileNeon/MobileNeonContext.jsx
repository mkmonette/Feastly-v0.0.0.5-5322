...
import { mobileNeonTokens } from '../mobileNeonTokens';
import { useProducts } from '@/context/ProductContext';

const MobileNeonContext = createContext();

export const useMobileNeon = () => {
  const context = useContext(MobileNeonContext);
  if (!context) {
    throw new Error('useMobileNeon must be used within MobileNeonProvider');
  }
  return context;
};
...
const defaultSectionsConfig = [
  ...
  },
  {
    ...
    content: {
      ...
      features: [
        ...
        { icon: 'safe', label: 'Safe' },
        { icon: 'free', label: 'Free' },
        { icon: 'fresh', label: 'Fresh' }
      ]
    }
  },
  {
    ...
    visibility: { enabled: true, devices: ['desktop', 'mobile'], conditions: [] },
    content: {
      items: ['24/7 DELIVERY', 'NEON DEALS', 'ORDER NOW']
    }
  },
  {
    id: 'featured',
  ...
  },
  {
    ...
    content: {
      emoji: '🌙',
      title: 'Night Owl?',
      subtitle: 'Get 15% off orders after midnight',
...
export const MobileNeonProvider = ({ children }) => {
  const [tokens] = useState(mobileNeonTokens);
  ...
  return (
    <MobileNeonContext.Provider value={value}>
      {children}
    </MobileNeonContext.Provider>
  );
};
