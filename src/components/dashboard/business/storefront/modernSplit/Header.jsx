        ...
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {['Menu', 'About', 'Gallery', 'Contact'].map((item) => (
              <button
                key={item}
                className={`${typography.scale.body} ${typography.weights.medium} transition-colors ${typography.fontSecondary}`}
          ...
          <button className="md:hidden">
            <SafeIcon icon={FiIcons.FiMenu} className="text-2xl" style={{ color: colors.textPrimary }} />
          </button>
        </div>
      </div>
    </header>
