import React, { createContext, useContext, useState } from 'react';
import { mobileCardMenuTokens } from '../mobileCardMenuTokens';
import { useProducts } from '@/context/ProductContext';

const MobileCardMenuContext = createContext();

export const useMobileCardMenu = () => {
  const context = useContext(MobileCardMenuContext);
  if (!context) {
    throw new Error('useMobileCardMenu must be used within MobileCardMenuProvider');
  }
  return context;
};

const defaultSectionsConfig = [
  {
    id: 'header',
    name: 'Header',
    visibility: { enabled: true, devices: ['desktop', 'mobile'], conditions: [] }
  },
  {
    id: 'hero',
    name: 'Hero',
    visibility: { enabled: true, devices: ['desktop', 'mobile'], conditions: [] },
    content: {
      preText: 'Welcome to Our Kitchen',
      headline: 'Fresh Food Delivered',
      headlineHighlight: 'Delivered',
      subtitle: 'Order your favorites and get them delivered fresh to your door',
      ctaPrimary: 'Start Ordering',
      ctaSecondary: 'See Menu',
      backgroundImage: 'https://images.pexels.com/photos/1633578/pexels-photo-1633578.jpeg'
    }
  },
  {
    id: 'categories',
    name: 'Category Tabs',
    visibility: { enabled: true, devices: ['desktop', 'mobile'], conditions: [] }
  },
  {
    id: 'featured',
    name: 'Featured Products',
    visibility: { enabled: true, devices: ['desktop', 'mobile'], conditions: [] },
    content: {
      title: 'Chef Specials',
      titleHighlight: 'Specials',
      subtitle: 'Handpicked favorites from our menu'
    }
  },
  {
    id: 'products',
    name: 'Product Grid',
    visibility: { enabled: true, devices: ['desktop', 'mobile'], conditions: [] },
    content: {
      title: 'Browse All Items',
      titleHighlight: 'All Items',
      subtitle: 'Explore our complete menu'
    }
  },
  {
    id: 'banner',
    name: 'Banner',
    visibility: { enabled: true, devices: ['desktop', 'mobile'], conditions: [] },
    content: {
      title: 'Limited Time Offer',
      titleHighlight: 'Offer',
      description: 'Get free delivery on orders over $30',
      buttonText: 'Order Now'
    }
  },
  {
    id: 'gallery',
    name: 'Gallery',
    visibility: { enabled: true, devices: ['desktop', 'mobile'], conditions: [] },
    content: {
      title: 'Our Kitchen',
      titleHighlight: 'Kitchen',
      images: [
        'https://images.pexels.com/photos/1640777/pexels-photo-1640777.jpeg',
        'https://images.pexels.com/photos/1279330/pexels-photo-1279330.jpeg',
        'https://images.pexels.com/photos/1633578/pexels-photo-1633578.jpeg'
      ]
    }
  },
  {
    id: 'testimonials',
    name: 'Testimonials',
    visibility: { enabled: true, devices: ['desktop', 'mobile'], conditions: [] },
    content: {
      title: 'What Our Customers Say',
      titleHighlight: 'Customers Say',
      testimonials: [
        {
          name: 'Emily Chen',
          role: 'Food Lover',
          content: 'Amazing quality and taste! The delivery is always on time.',
          image: 'https://images.pexels.com/photos/774909/pexels-photo-774909.jpeg'
        }
      ]
    }
  },
  {
    id: 'cta',
    name: 'CTA Banner',
    visibility: { enabled: true, devices: ['desktop', 'mobile'], conditions: [] },
    content: {
      title: 'Hungry? Order Now',
      titleHighlight: 'Order Now',
      subtitle: 'Fast delivery to your location',
      buttonText: 'Browse Menu'
    }
  },
  {
    id: 'contact',
    name: 'Contact',
    visibility: { enabled: true, devices: ['desktop', 'mobile'], conditions: [] },
    content: {
      title: 'Contact Us',
      titleHighlight: 'Contact',
      phone: '(555) 987-6543',
      email: 'hello@restaurant.com',
      address: '456 Food Street, Culinary City'
    }
  },
  {
    id: 'footer',
    name: 'Footer',
    visibility: { enabled: true, devices: ['desktop', 'mobile'], conditions: [] }
  }
];

export const MobileCardMenuProvider = ({ children }) => {
  const [tokens, setTokens] = useState(mobileCardMenuTokens);
  const [overrideTokens, setOverrideTokens] = useState({});
  const [sectionsConfig, setSectionsConfig] = useState(defaultSectionsConfig);
  const [previewDevice, setPreviewDevice] = useState('mobile');
  const [cartItems, setCartItems] = useState([]);
  const [isCartOpen, setIsCartOpen] = useState(false);
  const { products } = useProducts();

  const mergedTokens = {
    colors: { ...tokens.colors, ...overrideTokens.colors },
    typography: { ...tokens.typography, ...overrideTokens.typography },
    layout: { ...tokens.layout, ...overrideTokens.layout },
    effects: { ...tokens.effects, ...overrideTokens.effects }
  };

  const updateSection = (sectionId, updates) => {
    setSectionsConfig(prev =>
      prev.map(section =>
        section.id === sectionId ? { ...section, ...updates } : section
      )
    );
  };

  const resetTokens = () => {
    setOverrideTokens({});
  };

  const saveTokens = () => {
    console.log('Saving tokens and sections config...', { overrideTokens, sectionsConfig });
  };

  const addToCart = (product) => {
    setCartItems(prev => {
      const existing = prev.find(item => item.id === product.id);
      if (existing) {
        return prev.map(item =>
          item.id === product.id ? { ...item, quantity: item.quantity + 1 } : item
        );
      }
      return [...prev, { ...product, quantity: 1 }];
    });
  };

  const removeFromCart = (productId) => {
    setCartItems(prev => prev.filter(item => item.id !== productId));
  };

  const updateCartQuantity = (productId, quantity) => {
    if (quantity <= 0) {
      removeFromCart(productId);
      return;
    }
    setCartItems(prev =>
      prev.map(item =>
        item.id === productId ? { ...item, quantity } : item
      )
    );
  };

  const cartTotal = cartItems.reduce((sum, item) => sum + (item.price * item.quantity), 0);
  const cartItemCount = cartItems.reduce((sum, item) => sum + item.quantity, 0);

  const value = {
    tokens: mergedTokens,
    overrideTokens,
    setOverrideTokens,
    sectionsConfig,
    updateSection,
    resetTokens,
    saveTokens,
    previewDevice,
    setPreviewDevice,
    products,
    cartItems,
    cartItemCount,
    cartTotal,
    addToCart,
    removeFromCart,
    updateCartQuantity,
    isCartOpen,
    setIsCartOpen
  };

  return (
    <MobileCardMenuContext.Provider value={value}>
      {children}
    </MobileCardMenuContext.Provider>
  );
};
