...
const ModernSplitCartStorefrontContext = createContext(null);
...
export const ModernSplitCartStorefrontProvider = ({ children }) => {
  const [overrideTokens, setOverrideTokens] = useState(() => {
    try {
      const saved = localStorage.getItem('modernSplitCartOverrideTokens');
      return saved ? JSON.parse(saved) : {};
    } catch (e) {
      console.error("Failed to load modern split cart tokens", e);
      return {};
    }
  });
  ...
  const [sectionsConfig, setSectionsConfig] = useState(() => {
    try {
      const saved = localStorage.getItem('modernSplitCartSectionsConfig');
      if (saved) {
      ...
      console.error("Failed to load modern split cart sections", e);
      return DEFAULT_SECTIONS;
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem('modernSplitCartOverrideTokens', JSON.stringify(overrideTokens));
    } catch (e) {
      console.error("Failed to save modern split cart tokens", e);
    }
  }, [overrideTokens]);

  useEffect(() => {
    try {
      localStorage.setItem('modernSplitCartSectionsConfig', JSON.stringify(sectionsConfig));
    } catch (e) {
      console.error("Failed to save modern split cart sections", e);
    }
  }, [sectionsConfig]);

  const tokens = useMemo(() => mergeTokens(modernSplitCartTokens, overrideTokens), [overrideTokens]);
  ...
  return (
    <ModernSplitCartStorefrontContext.Provider value={value}>
      {children}
    </ModernSplitCartStorefrontContext.Provider>
  );
};

export const useModernSplitCartStorefront = () => {
  const context = useContext(ModernSplitCartStorefrontContext);
  if (!context) {
    throw new Error('useModernSplitCartStorefront must be used within ModernSplitCartStorefrontProvider');
  }
  return context;
};
