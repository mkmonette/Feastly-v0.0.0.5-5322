...
const Testimonials = () => {
  ...
  return (
    <section className={`py-8 ${layout.horizontalPadding}`} style={{ backgroundColor: colors.surface }}>
          ...
          {testimonials.map((testimonial, index) => (
            ...
            >
              <div className="flex items-center gap-1 mb-3">
                {[...Array(5)].map((_, i) => (
                  <SafeIcon key={i} icon={FiIcons.FiStar} className="text-sm" style={{ color: colors.accent, fill: colors.accent }} />
                ))}
              </div>

              <p className={`${typography.scale.bodySmall} ${typography.lineHeights.relaxed} mb-4 ${typography.fontSecondary}`} style={{ color: colors.textSecondary }}>
                {testimonial.text}
              </p>

              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full overflow-hidden" style={{ backgroundColor: colors.surfaceAlt }}>
                  {testimonial.avatar ? (
                    <img src={testimonial.avatar} alt={testimonial.name} className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center">
                      <SafeIcon icon={FiIcons.FiUser} className="text-lg" style={{ color: colors.textMuted }} />
                    </div>
                  )}
                </div>
                <div>
